...
function loadshp(config, returnData, errorCallback) {
  url = config.url;
  encoding = typeof config.encoding != 'utf-8'
    ? config.encoding
    : 'utf-8';
  EPSG = typeof config.EPSG != 'undefined'
    ? config.EPSG
    : 4326;
  EPSGMAPA = typeof config.EPSGMAPA != 'undefined'
    ? config.EPSGMAPA
    : 'EPSG:3857';
...
function toGeojson(geojsonData) {
  ...
  for (var i = 0; i < shpRecords.length; i++) {
    ...
    switch (shpRecords[i].shape.type) {
      ...
      case 3:
      case 8:
        geometry.type = (shpRecords[i].shape.type == 3
          ? "LineString"
          : "MultiPoint");
